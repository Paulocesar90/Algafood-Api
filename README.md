<b>Algafood-api</b>
![ESR](https://user-images.githubusercontent.com/102321311/197084460-838fcd4b-d904-4528-bc44-ccaaf534e478.png)
# Especialista Spring Rest
![Spring](https://static.computerworld.com.pt/media/2010/09/Spring-Logo.png)

Curso Especialista Spring da AlgaWorks. <br>
Pastas:
 - algafood-api: API principal de exploração das funcionalidades
 - client: Consumidor JavaScript da API principal (algafood-api)
 - algafood-java-client: Consumidor Java da API principal (algafood-api)
 ...
 - algafood-api/EspecialistaSpringRest.postman_collection.json: requisições CURL da API principal.
...
`java -jar target/algafood-api.jar --server.port=8081`
...
`java -jar target/algafood-api.jar --spring.profiles.active=development`
...
Acessar console (https://s3.console.aws.amazon.com/), serviços, S3 e criar <b>Bucket</b> ("algafood-paulo").
Acessar serviço "IAM" e adicionar <b>usuário</b> ("algafood-test-s3"). Habilitar "Acesso programático".
...
```
Navegador indica que o dado foi recuperado do cache local: ![cache](/algafood-api/images/cache.png)
    ...
    public Docket apiDocket() {
        return new Docket(DocumentationType.SWAGGER_2).select()
                .apis(Predicates.and( 	// Documenta controllers dos pacotes especificados (mais de um)
                        ...
                        RequestHandlerSelectors.basePackage("com.algafood.api.outro"))
                )
                .apis(RequestHandlerSelectors.basePackage("com.algafood.api")) // Documenta controllers do pacote específico
...
Técnicas de versionamento de APIs:
 - Versionamento por Media Type: 
  Cliente envia o número da versão no header *Accept*: `Accept=application/vnd.algafood.v1+json`
  Api prepara o controller, no exemplo: `@RequestMapping(path = "/cidades", produces = application/vnd.algafood.v1+json)`
...
#### Papéis (Roles)
 - Resource Server: servidor que hospeda os recursos protegidos, a API (algafood API)
 ...
 - Authorization Server: Servidor que autentica o Resource Owner e garante autorização de acesso para o cliente ao Resource Server (algafood Auth). 
...
Exemplo de login joao.ger@kalgafood.com, senha: 123. Link de acesso no navegador para [login](http://localhost:8080/oauth/authorize?response_type=code&client_id=foodanalytics&state=abc&redirect_uri=http://localhost:3000) do usuário.
...
`"username":"paulo", "password": 123", "grant_type": password"`
...
| algafood-web  | web123  | password  |
| foodanalytics  | algafood123  | authorization_code  |
...
Para autenticação na documentação veja imagem do [formulário de autenticação](/algafood-api/images/security-formulario-autorizacao-documentacao-swagger.png).
...
Endpoint chamado para verificar token JWT, expõe a chave pública, inicialmente extraindo do arquivo _algafood.jks_ na função `keyPair()`. Posteriormente extraido do arquivo em bytes criado a partir de codigo base64. Deve possuir a nomenclatura path padrão:
`http://localhost:8080/.well-known/jwks.json`

##### .jks
Comando no local da pasta "resource/keys" é possível obter o arquivo algafood.jks codificado por Base64:
 > cat algafood.jks | base64 // unit
 > type algafood.jks | openssl base64 // windows
...
[documentação sobre commandline](https://docs.docker.com/engine/reference/commandline/docker/).
 ...
 - `docker container run -p 80:80 -d --name blogalgafood wordpress` - cria container com nome definido publicando porta local 80 na porta 80 do container
 - `docker container run -p 80:80 -d --rm --name blogAlgafood wordpress`- parâmetro <b>--rm</b> indica que será removido automaticamente quando o container é parado
 ...
 - `docker container run -d -p 3306:3306 -e MYSQL_ALLOW_EMPTY_PASSWORD=yes --name algafood-mysql mysql:8.0` - inicializa container MySql, parâmetro <b>-e</b> habilita passar variável de ambiente *MYSQL_ALLOW_EMPTY_PASSWORD* com valor yes, permitindo que usário root tenha senha vazia.
 - `docker container run -d -p 3307:3306 -e MYSQL_ROOT_PASSWORD=root --name algafood-mysql mysql` - instancia container na ultima versão do MySql, com password `root` acessado localmente na porta 3307
...
Para criar imagem, na pasta do projeto algafood-api, executar: `docker image build -t algafood-api .` Onde:
...
Instancia um container utilizando a última versão do MySql, com password `root` para usuário `root`, acessado localmente através da porta 3307: `docker container run -d -p 3307:3306 -e MYSQL_ROOT_PASSWORD=root --name algafood-mysql mysql`.
...
Comando para conectar container MySQL na rede algafood-network:
`docker container run -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root --network algafood-network --name algafood-mysql mysql`.

<b>2. Container API</b>
Executando container da API com variável de ambiente *DB_HOST* correspondente ao container MySQL, conectada à rede *algafood-network*:
...
```xml
<configuration>
	<repository>algafood-api</repository> <!-- Nome da Imagem -->
...
Localmente:
 - Alterando a tag da imagem local para adicionar o DockerID, para que o Docker Hub identifique o user - `docker image tag algafood-api:latest <dockerID>/algafood-api:latest`.
 - Login - `docker login`
 - Envia a aimagem para DockerHub - `docker push algafood-api:latest`
 - Baixando e executando imagem do Docker Hub - `docker container run --rm -p 8080:8080 -e DB_HOST=algafood-mysql --network algafood-network algafood-api`
...
Para verificar isso podemos instanciar um container com distribuição Linux Alpine na mesma rede, para que possamos acessá-lo pelo seu terminal, a fim de acessar os containers do algafood-api, onde `algafood-api_algafood-network` é a rede criada pelo docker Compose:
`docker container run --rm -it --network algafood-api_algafood-network alpine sh`. 

 - No terminal iterativo, consultar o servidor DNS por nome: `nslookup algafood-api`
 - Instalar o curl: `apk add curl`
 - Acessar endpoint e verificar IPs diferentes sendo acessados: `curl http://algafood-api:8080/hostcheck`.
...
Iniciando container cliente do Redis para acessá-lo: `docker container run --rm -it --network algafood-api_algafood-network redis:6.2.1-alpine sh`. Em seguida, o comando `redis-cli -h algafood-redis` dá acesso ao prompt redis-cli, sendo possível visualizar suas chaves armazenadas.
...
No docker-compose as seguintes variáveis de ambiente substituem `spring.session.store-type=none` e `spring.redis.host=algafood-redis` localizadas em *application.properties*.
...
Na classe *StorageConfig* inserimos uma condição, apenas se `algafood.storage.tipo=s3` são carregadas as demais (algafood.storage.s3.id-chave-acesso e algafood.storage.s3.chave-acesso-secreta), assim é possível que em desenvolvimento estas propriedades estejam ausentes.
...
Ao selecionar o serviço `RDS`, acessar no menu lateral "Banco de Dados" e "Criar banco de dados". Na opção Modelos escolher "Free tier" ou "Nível gratuito". Em "Identificador da instância de banco de dados" insira o nome do banco de dados: algafood-mysql.
...
"Grupo de segurança" funciona como um firewall virtual, controlando entrada e saída, por exemplo podemos configurar IPs que podem acessar o DB. Selecione Criar novo e identifique com "algafood-mysql-sg".

#### VPC
Selecionar o serviço VPC, Security, Security Groups é possível verificar o `algafood-mysql-sg`, e que o Ip de entrada permitido é o IP do meu usuário ao criar o banco de dados. É possível alterar essa configuração em "Edit Inbound rules" ou "Editar regras de entrada".

Acessando o banco de dados via Dbeaver, criamos o schema "algafood", e o usuário "algafood-api" com todos os provilégios para o banco *algafood-api*. O host e porta ficam visíveis ao acessar o serviço RDS > Banco de Dados, clicar sobre o nome do banco é possível visualizar Endpoint e porta.
...
Acessar no menu lateral `Amazon ECS > Cluster`, create cluster, Network Only. EM cluster name inserimos *algafood-cluster*, não é necessário criar a VPC, selecionar *create*. `Services` no contexto do AWS EC2, é um serviço que mantém uma task continuamente em execução, caso ocorra um problema no container e ele caia, o service o reinicia. Ao visualizar o cluster selecionar a tab Service e criar um service. Selecione o cluster VPC que aparece nas opções e editar  `Security Group`, alteramos o nome para "nginx-sg". "Auto-assign public IP" enabled pois desejamos acessar o container de forma externa através da Internet.
...
Criar novo repositório, privado, nomeamos como `algafood-api`, Create repository. Para inserir imagem selecione o repositório criado e "View push commands", serão exibidos comandos para inserir imagens no Registry. Necessário instalar <b>AWS Command Line Interface</b>. 
...
Criamos a imagem docker do projeto com `mvn package -Pdocker` para realizar upload dela no ECR. Vamos tagea-lá segundo comando disponível no ECR:
 > docker tag algafood-api:latest ************.dkr.ecr.us-east-2.amazonaws.com/algafood-api:latest

 E, seguida realizamos o push da imagem para Amazon ECR:
 > docker push ********.dkr.ecr.us-east-2.amazonaws.com/algafood-api:latest
...
Para dar permissão de acesso ao banco selecione o serviço da AWS "VPC", aba no menu lateral "Security Groups", Editar regras de entrada, adicionar nova regra. No campo "Origem" buscamos e selecionamos "algafood-api-sg", desse modo damos permissão para o grupo `algafood-api-sg` de acesso ao banco de dados.
...
Criamos novo <b>Target Group</b> (Grupo de Destino) chamado "algafood-api-service-tg", para agrupar as instâncias em execução dentro do serviço algafood-service. Em "target type" selecionamos IP, pois a forma com que o ECS s eintegra com o Load Balancer é através do IP, caso fossem instâncias do EC2 deveria ser selecionado "Instance". Selecionamos Http, 80.
...
"Create new security group" e criamos "algafood-lb-sg", com regra de entrada TCP, porta 80, de qualquer lugar. Create! Podemos acessar através do nome do DNS, porém recebemos 503 pois ainda não há targets no grupo.

##### Configurando o balanceamento de carga no Service do Amazon ECS
Para adicionar um Load Balancer no Service, devemos fazer isso na criação do Service. Excluímod o "algafood-aservice" e criamos o `algafood-api-service`, com duas tasks para utilizar o balanceamento. Selecionando pelo menos duas subnets (zonas de disponibilidade). Editamos o Security group e selecionamos o previamente criado `algafood-api-service-sg`.
...
Em `Production listener port`selecionamos *80:HTTP*, e selecionamos o Target group "algafood-api-service-tg". Next > create service.
...
Para que a comunicação fique limitada ao acesso via Load balancer vamos remover a possibilidade de acessar diretamente os containers. Acessamos `VPC > Security Groups > algafood-api-service` > Regras de entrada, e excluímos a regra de entrada anterior e adicionamos a regra de entrada apenas para o Balancer `TCP:8080, algafood-lb-sg`.
...
No nome de domínio inserimos: *.algafood.com.br.
...
Clicamos no listener a adicionamos nova Regra (new rule). Host header:algafood.com.br, target group:algafood-api-service-tg.
Após isso temos dois listeners, na porta 80 e outro na 443, caso não usado o na porta 80 pode sre excluído.

Editar o security group de algafood-lb-sg, editamos a regra de entrada para permitir acesso ao protocolo HTTPS, TCP, 443 de qualquer lugar.
...
Após isso devemos empacotar novamente a aplicação com `mvn package -Pdocker`, acessar o <b>AWS Elastic Container Registry</b>, em Repositories, e capturar o comandos push, executar localmente para fazer upload da imagem. Em seguida acessamos os clusters em ECS no menu lateral, selecionamos o service `algafood-service`, habilitamos Force new deployment, para que seja atualizado para a nova imagem do container.
...
[1]: https://www.linkedin.com/in/paulo-c-p-costa/
[3]: https://github.com/Paulocesar90
